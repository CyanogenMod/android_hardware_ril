...
processCommandBuffer(void *buffer, size_t buflen, RIL_SOCKET_ID socket_id) {
    ...
    if (request < 1 || request >= (int32_t)NUM_ELEMS(s_commands)) {
        Parcel pErr;
        RLOGE("unsupported request code %d token %d num_elem %d", request, token, (int32_t)NUM_ELEMS(s_commands));
...
static void dispatchsetMaxTransmitPower(Parcel &p, RequestInfo *pRI) {
    ...
    status = p.readInt32(&t);
    if (status != NO_ERROR) {
        RLOGE("dispatchsetMaxTransmitPower ERROR");
        goto invalid;
    }
    state.state = (int)t;
    RLOGI("dispatchsetMaxTransmitPower: %d\n", state.state);
    ...
    s_callbacks.onRequest(pRI->pCI->requestNumber, &state, sizeof(state), pRI);
